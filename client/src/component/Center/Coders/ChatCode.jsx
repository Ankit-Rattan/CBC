  ...
  return (
    ...
    <p></p>
      <div className="chat h-[70vh] overflow-scroll ">
        {messages.map((message, index) => (
          <div key={index} className={message.user === user ? "my-mess" : "other-mess"} >
            <strong className="text-right" >{message.user}:</strong> {message.message}
          </div>
        ))}
      </div>
    </div>
    <div className="flex flex-wrap gap-1 justify-center mt-[-1rem] ">
      ...
      <input
        ...
        className="w-[33%] rounded-full p-1 shadow-md"
      />
      <button className="w-[10%] bg-green-300 rounded-full shadow-md" onClick={sendMessage}>Send</button>
    </div>
    </div>
  );
};
